...
from src.models import train_bnn_for_calibration
...
BLUEBEAR_MAX_PROBLEM_WORKERS = int(os.environ.get("BLUEBEAR_MAX_PROBLEM_WORKERS", "10"))
...
surrogate_config = experiment_config["surrogates"]["bnn"]
method_name = "BNN"
...
bnn_hidden = tuple(surrogate_config["hidden"])
bnn_lr = surrogate_config["lr"]
bnn_max_steps = surrogate_config["max_steps"]
bnn_patience = surrogate_config["patience"]
bnn_fixed_sigma = surrogate_config["fixed_sigma"]
...
def train_model_for_calibration(problem, sample_size, train_seed=42, test_seed=1):
    return train_bnn_for_calibration(
        problem=problem,
...
def run_problem(problem_name):
    ...
    model_f1, model_f2, X_train, y_train, f_train_mean, X_test, y_test = train_model_for_calibration(
        problem=problem,
    ...
    for optimizer_spec in optimizer_run_specs:
        ...
        if optimizer_spec["use_dual_ranking"]:
            if dual_ranking_survival is None:
                print(f"Dual-ranking upper quantile: q={dual_ranking_quantile:.2f}")
    ...
    return problem_name, problem_summary_rows
...
def _ordered_result_summary_rows():
    rows = []
...
def _record_completed_problem(completed_problem_name, problem_summary_rows):
    all_results[completed_problem_name] = None
    summary_rows_by_problem[completed_problem_name] = list(problem_summary_rows)
    result_summary_rows[:] = _ordered_result_summary_rows()
    write_result_summary_temp(
        result_summary_rows,
        output_dir=OUTPUT_DIR,
        method_name=method_name,
...
max_problem_workers = min(BLUEBEAR_MAX_PROBLEM_WORKERS, len(problem_names))
print(f"BlueBEAR problem-level parallel workers: {max_problem_workers}")
if max_problem_workers <= 1:
    for problem_index, problem_name in enumerate(problem_names, start=1):
        completed_problem_name, problem_summary_rows = run_problem(problem_name)
        _record_completed_problem(completed_problem_name, problem_summary_rows)
else:
    with ProcessPoolExecutor(max_workers=max_problem_workers) as executor:
        future_to_problem = {
            executor.submit(run_problem, problem_name): problem_name
            for problem_name in problem_names
        }
        for future in as_completed(future_to_problem):
            problem_name = future_to_problem[future]
            completed_problem_name, problem_summary_rows = future.result()
