        ...
        for strat = 1:length(Strategies)
            ...
            for i=1:Runs
                ...
                elseif Design == "MVNORM"
                   ...
                   mean_pnt_1 = Boundary(2,1)+((Boundary(1,1) - Boundary(2,1))/4);
                   mean_pnt_2 = Boundary(2,1)+((Boundary(1,1) - Boundary(2,1))*3/4);
                   
                   lb = Boundary(2,1)-mean_pnt_1;
                   ub = Boundary(1,1)-mean_pnt_1;
                   %mu = mean_pnt*ones(1,nvars);
                   sigma = 0.0*ones(no_vars,no_vars)+0.1*eye(no_vars); 
                   Population_1 = mvrandn(lb*ones(1,no_vars),ub*ones(1,no_vars),sigma,sample_size/2)+mean_pnt_1;
                   
                   lb = Boundary(2,1)-mean_pnt_2;
                   ub = Boundary(1,1)-mean_pnt_2;
                   %mu = mean_pnt*ones(1,nvars);
                   sigma = 0.0*ones(no_vars,no_vars)+0.1*eye(no_vars); 
                   Population_2 = mvrandn(lb*ones(1,no_vars),ub*ones(1,no_vars),sigma,sample_size/2)+mean_pnt_2;            
