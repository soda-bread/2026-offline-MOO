...
class Prob_APD_select_v3(SelectionBase):
    ...
    def do(self, pop: Population, vectors: ReferenceVectors) -> List[int]:
        ...
        refV = vectors.neighbouring_angles_current.copy()
        refV[~np.isfinite(refV) | (refV <= np.finfo(float).eps)] = np.finfo(float).eps
        ...
        fitness_norm[
            ~np.isfinite(fitness_norm) | (fitness_norm <= np.finfo(float).eps)
        ] = np.finfo(float).eps
        ...
        normalized_fitness = np.nan_to_num(normalized_fitness, nan=0.0, posinf=1.0, neginf=0.0)
